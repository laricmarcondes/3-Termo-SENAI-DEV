...
function ECadastrarVagas() {
    ...
    function renderHabilidades() {
        ...
        return vagaForm.habilidadeVaga?.map((item: HabilidadeVaga) => {
            return (
                <a className="bg-gray-400 p-2 rounded-full m-1 h-8 flex items-center cursor-pointer"
                    onClick={() => removeHabilidade(item.idHabilidade!)}>
                        {habilidades?.filter(h => h.idHabilidade == item.idHabilidade)[0].nomeHabilidade}
                </a>
            )
        })
    }
        ...
        <div className="body w-full">
            ...
            <h1 className="p-10 md:text-2xl text-xl flex justify-center">Cadastrar Vaga</h1>
                ...
                <form className="w-1/2 rounded-md lg:w-1/2 m-auto w-full"
                    ...
                    <div className="flex">
                        <Input className="flex-1" placeholder='Nome do Título' name="título" label="Titulo"
                            onChange={e => setVagaForm({ ...vagaForm, titulo: e.target.value })} required
                            value={vagaForm.titulo}></Input>
                        <Input className="w-32 ml-4" placeholder='1000,00' name="salário" label="Salário"
                    ...
                    <div className="flex mt-1">
                        <Input className="flex-1 mt-3" placeholder='00001001' name="cep"
                            label="Cep"
                        ...
                        <Input className="flex-1 ml-4 mt-3" name="cep" label="Data de expiração"
                            type="Date"
                            onChange={e => setVagaForm({ ...vagaForm, dataExpiracao: e.target.value })} required
                            value={vagaForm.dataExpiracao}></Input>
                    </div>
                    <div className="flex mt-1 mb-1">
                        <Input className="flex-1 mt-3" placeholder='Programador Jr' name="Qualificação" label="Qualificação"
                            onChange={e => setVagaForm({ ...vagaForm, qualificacao: e.target.value })} required
                            value={vagaForm.qualificacao}></Input>
                        <Input className="w-32 ml-4 mt-3" placeholder='6 Horas' name="Carga Horária" label="Carga Horária"
                    ...
                    <div className="flex items-end">
                        ...
                        <Button className="ri-add-line ml-2"
                    ...
                    <div className="flex items-end">
                        ...
                        <Button className="ri-add-line ml-2" type="button"
